...
- [ ] Backlog: батч-эндпоинт embedding API или параллельные запросы к нему для перезаливок — сейчас serial ~4 rec/s, 14.5К точек = ~64 мин простоя поиска (см. риски)
...
| Полная перезаливка Qdrant payload | Идемпотентный backfill-скрипт (уже обкатан); факт по Ф0: ~64 мин при 14.5К точек — узкое место embedding API (~4 rec/s serial); «минуты» достижимы только при прогретом кеше эмбеддингов — запускать в окно; расхождение ловит reconciliation из Ф0 |
