...
void main() {
  ...
  // яркая привязка к кромке (диск не пересвечиваем) + мягкий спад ^1.6
  float atmoRise = smoothstep(0.42, 0.60, r);
  float atmoFall = pow(1.0 - smoothstep(0.45, 1.0, r), 1.6);
  float atmo = atmoRise * atmoFall;

  // ── ВСПОЛОХИ: угловой+временной шум (3 гармоники k=2,3,5), ±40% —
  // пламя «дышит» несимметрично
  float flare =
    sin(ang * 2.0 + uTime * 0.9 + vSeed * 6.2831) * 0.45 +
    sin(ang * 3.0 - uTime * 0.7 + vSeed * 3.1) * 0.35 +
    sin(ang * 5.0 + uTime * 1.3 + vSeed * 1.7) * 0.20;
  atmo *= 1.0 + 0.40 * flare;
  ...
  // цвет атмосферы: слой с нагревом к белому у кромки
  vec3 atmoTint = mix(vLayerColor, vec3(1.0), 0.35 * atmoRise);

  float alpha = (atmo * 0.65 + ring * 0.8 + outer * 0.25) * intensity * flicker;
