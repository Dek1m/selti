CREATE FUNCTION public.graph_traverse_full(p_start_id uuid, p_depth integer DEFAULT 3, p_link_types text[] DEFAULT NULL::text[], p_as_of timestamp with time zone DEFAULT now()) RETURNS TABLE(nodes jsonb, edges jsonb)
    LANGUAGE sql STABLE
    AS $$
