...
## prune_granulation_session.py

Чистка сессии ежечасной автоматизации грануляции (решение Мастера
20.09): recurring-автоматизация живёт в одной сессии ZCode, и каждый
часовой тик добавляет в неё сообщения — сессия росла вечно. Скрипт
оставляет в ней только последние `--keep` (default 10) тиков:

- находит сессию в `~/.zcode/cli/db/db.sqlite` по title,
- тик = user-сообщение с маркером промпта автоматизации,
- удаляет старые строки из `message`/`part` и телеметрии
  (`turn_usage`/`model_usage`/`tool_usage`) по `started_at` cutoff,
- перед удалением архивирует строки в
  `~/.zcode/cli/prune-backups/pruned-*.json.gz` (хранить 5 последних),
- БД не вакуумится — освободившиеся страницы переиспользуются, файл
  перестаёт расти без дорогого VACUUM под живым клиентом.

Вызывается последним шагом промпта автоматизации (см. CronList),
вручную: `python prune_granulation_session.py [--dry-run] [--keep 10]`.
